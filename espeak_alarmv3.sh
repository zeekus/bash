...
##################
  ...
  while true; do
    ...
    echo "$alarm_type alarm" | espeak -a 500 -p 1
    date +"at  %l:%M%p" | espeak -a 500 -p 1
...
##################
  ...
  echo "use: '$0 l #for lunch"
  echo "use: '$0 c #for lunch"
  exit 0
}
...
if [ $use_seconds == 1 ] ; then  
  ...
  if [[ $alarm_type = 'standard' ]]; then
    ...
    echo "2. $alarm_type "
    echo "...test countdown"
    time_at_end_of_alarm=$(date --date="+$alarm minutes +5 seconds" +"%s")
    hr_time_at_end_of_alarm=$(date --date="+$alarm minutes +5 seconds" +"%l:%M:%S")
  else
    echo "2. $alarm_type "
...
do 
 ...
 ##############
 echo "We have exactly ($remaining_seconds)s left"
 if [ $remaining_seconds -gt 30 ]; then 
  long_sleep_wait=30
  sleep $long_sleep_wait
 elif [ $remaining_seconds -gt 5 ]; then 
  long_sleep_wait=5
  sleep $long_sleep_wait
 else
   remaining_seconds=$(( $time_at_end_of_alarm - $current_time ))
   echo "..count down  $remaining_seconds"
   #echo "$remaining_seconds" | espeak -a 500 -p 1 -s 60 
   echo "$remaining_seconds" | espeak -a 500 -s 130 
 fi
